...
function rpc(fn,payload){return fetch(SUPA+'/rest/v1/rpc/'+fn,{method:'POST',headers:{apikey:KEY,Authorization:'Bearer '+KEY,'Content-Type':'application/json'},body:JSON.stringify(payload)}).then(function(r){return r.text().then(function(t){var j=null;try{j=t?JSON.parse(t):null}catch(e){}if(!r.ok)throw Error(j&&j.message?j.message:'문제가 생겼어요.');return j})})}
function refresh(){if(!session)return Promise.reject(Error('대화틈을 선택해 주세요.'));return rpc('dt_get_state',{p_room_id:session.room,p_participant_token:session.token}).then(function(x){var before=state?JSON.stringify([state.round_id,state.partner&&state.partner.answered,state.unlocked,state.history&&state.history.length]):'';state=x;session.name=x.me&&x.me.name?x.me.name:session.name;session.partnerName=x.partner&&x.partner.name?x.partner.name:session.partnerName;session.lastRound=x.round_sequence||session.lastRound;session.lastSeen=new Date().toISOString();saveSession(session);var after=JSON.stringify([x.round_id,x.partner&&x.partner.answered,x.unlocked,x.history&&x.history.length]);return {state:x,changed:before!==after}})}
function createRoom(name){return rpc('dt_create_room',{p_creator_name:name}).then(function(r){var x=Array.isArray(r)?r[0]:r;saveSession({room:x.room_id,token:x.participant_token,invite:x.invite_token,name:name,rev:[]});return refresh()})}
...
window.DT={esc:esc,nl:nl,initial:initial,base:base,spaces:function(){return registry.spaces.slice()},session:function(){return session},state:function(){return state},setState:function(x){state=x},saveSession:saveSession,clearActive:clearActive,activate:activate,findByInvite:findByInvite,refresh:refresh,createRoom:createRoom,joinRoom:joinRoom,answer:answer,next:next,reveal:reveal,revealed:revealed,inviteUrl:inviteUrl,clearQuery:clearQuery,dateObj:dateObj,dateLabel:dateLabel,dayKey:dayKey};
})();
