...
function refresh(){if(!session)return Promise.reject(Error('No active space.'));return rpc('dt_get_state',{p_room_id:session.room,p_participant_token:session.token}).then(function(x){var before=refreshSignature(state);state=x;session.name=x.me&&x.me.name?x.me.name:session.name;session.partnerName=x.partner&&x.partner.name?x.partner.name:session.partnerName;session.memberNames=(x.participants||[]).map(function(p){return p.name});session.lastRound=x.round_sequence||session.lastRound;session.relationshipType=x.relationship_type||session.relationshipType||'partner';session.targetParticipants=x.max_participants||session.targetParticipants||2;session.isPremium=!!x.is_premium;session.lastSeen=new Date().toISOString();saveSession(session);var after=refreshSignature(x);return {state:x,changed:before!==after}})}
function createRoom(name,relationshipType,targetParticipants){var rel=relationshipType||'partner',target=parseInt(targetParticipants||window.DT_CREATE_TARGET||2,10);if(target<2||target>5)target=2;var premium=isPremium();return rpc('dt_create_room_v4',{p_creator_name:name,p_relationship_type:rel,p_target_participants:target,p_premium:premium}).then(function(r){var x=Array.isArray(r)?r[0]:r;window.DT_CREATE_TARGET=null;saveSession({room:x.room_id,token:x.participant_token,invite:x.invite_token,name:name,relationshipType:rel,targetParticipants:target,isPremium:premium,inviteSentAt:null,rev:[]});return refresh()})}
...
function next(force){return rpc('dt_start_next_round_v2',{p_room_id:session.room,p_participant_token:session.token,p_force:!!force}).then(function(x){state=x;session.lastRound=x.round_sequence||session.lastRound;saveSession(session);return x})}
function customQuestion(q,force){return rpc('dt_start_custom_round',{p_room_id:session.room,p_participant_token:session.token,p_question:q,p_force:!!force}).then(function(x){state=x;session.lastRound=x.round_sequence||session.lastRound;saveSession(session);return x})}
function enablePremium(){setPremium(true);if(!session||!state)return Promise.resolve(state);if(state.is_premium)return Promise.resolve(state);return rpc('dt_enable_premium',{p_room_id:session.room,p_participant_token:session.token}).then(function(x){state=x;session.isPremium=true;saveSession(session);return x})}
...
function markInviteSent(){if(!session)return;session.inviteSentAt=new Date().toISOString();saveSession(session);log('invite_sent',{target:state&&state.max_participants||session.targetParticipants||2,joined:state&&state.participant_count||1})}
function inviteWasSent(){return !!(session&&session.inviteSentAt)}
