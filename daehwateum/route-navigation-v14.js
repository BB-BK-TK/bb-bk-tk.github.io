...
var nativeOneShot=/DaehwateumAndroid\/0\.9/.test(ua);
...
function nativeTopOnce(){
  if(!nativeOneShot)return;
...
function stopEntryReset(){
  if(entryResetRaf){try{cancelAnimationFrame(entryResetRaf)}catch(e){}entryResetRaf=0}
  unlockAnchoring();
}
...
function cancelEntryResetOnInteraction(){if(entryResetRaf)stopEntryReset()}
['pointerdown','touchstart','wheel'].forEach(function(type){window.addEventListener(type,cancelEntryResetOnInteraction,{capture:true,passive:true})});

function ensureReturnStyle(){
  if(document.getElementById('dt-route-return-v14-style'))return;
  var s=document.createElement('style');
  s.id='dt-route-return-v14-style';
...
function homePatchReady(){var hero=app.querySelector('.hero');if(!hero)return false;return hero.hasAttribute('data-home-v9')||Date.now()-returnStartedAt>=180}
function endReturnReveal(force){
  ...
  requestAnimationFrame(function(){
    ...
    returningHome=false;returnSettling=false;clearReturnTimers();unlockAnchoring();
  });
}
...
function restoreHome(){
  ...
  var target=returnTarget();setY(target);
...
function beginReturnHome(){
  ...
  clearReturnTimers();ensureReturnStyle();
  returningHome=true;returnStartedAt=Date.now();returnSettling=false;mode='return-home';
  lockAnchoring();document.documentElement.classList.add('dt-route-returning');setY(returnTarget());
...
app.addEventListener('click',function(ev){
  ...
  if((action==='create'||action==='new')&&homeVisible()){
    homeY=currentY();hasHomeY=true;mode='enter-fresh';prepareSynchronousFreshRoute();
    try{node.blur()}catch(e){}
    return;
  }
  if(action==='open-room'&&homeVisible()){
    homeY=currentY();hasHomeY=true;mode='enter-room';
    try{node.blur()}catch(e){}
    return;
  }
...
new MutationObserver(function(){
  if(mode==='enter-fresh'&&freshScreenVisible()){
    mode=null;finishFreshTop(freshScreenVisible);return;
  }
  if(mode==='enter-room'&&roomVisible()){
    mode=null;scheduleRoomTop();return;
  }
  if(mode==='return-home'&&homeVisible()){
    mode=null;restoreHome();return;
  }
  if(returningHome&&homeVisible()){
    setY(returnTarget());if(homePatchReady())restoreHome();
  }
