...
function coreReady(){
  if(isHome()||app.querySelector('.dt-route-v8-loader'))return false;
  if(!app.querySelector('.top'))return false;
  return !!app.querySelector('.q,.stage,.intro,.question-queue-screen');
}
...
function timingSettled(){var gate=app.querySelector('.next-gate');if(!gate)return true;if(gate.classList.contains('timing-ready')||app.querySelector('.schedule-open'))return true;return coreSeenAt>0&&Date.now()-coreSeenAt>700}
...
function destinationSettled(){return coreReady()&&queueSettled()&&timingSettled()&&originSettled()&&Date.now()-lastMutation>=120}
...
function loaderMarkup(){var safe=roomName.replace(/[&<>"']/g,function(x){return{'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[x]});return'<div class="w dt-route-v8-loader" aria-hidden="true"><header class="top"><b class="brand">◉ 대화틈</b></header><section class="card" style="min-height:210px"><span class="k">대화를 여는 중</span>'+(safe?'<h2 style="margin-top:14px">'+safe+'</h2>':'')+'<p>잠시만 기다려주세요.</p></section></div>'}
...
function reveal(){if(!active||!coreReady()||!destinationSettled())return;setTop();requestAnimationFrame(function(){setTop();cleanup()})}
...
new MutationObserver(function(){if(!active)return;lastMutation=Date.now();if(coreReady()&&!coreSeenAt)coreSeenAt=Date.now();if(settleTimer)clearTimeout(settleTimer);settleTimer=setTimeout(check,120)}).observe(app,{childList:true,subtree:true,attributes:true,attributeFilter:['class','data-sig','data-queue-visual','hidden']});
})();
