...
function scheduleRoomTop(){
  ...
  lockAnchoring();
  forceRoomTop();
  // One final paint-aligned correction only. Any user gesture cancels it.
  entryResetRaf=requestAnimationFrame(function(){
    entryResetRaf=0;
    if(roomVisible())forceRoomTop();
    unlockAnchoring();
  });
}
...
function ensureReturnStyle(){
  if(document.getElementById('dt-route-return-v12-style'))return;
  var s=document.createElement('style');
  s.id='dt-route-return-v12-style';
