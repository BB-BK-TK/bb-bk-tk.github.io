...
function enhance(forceFeedback){syncWebState();var overlay=document.getElementById('settings-page-overlay');if(!overlay)return;var card=overlay.querySelector('.notification-settings-card');if(!card)return;var on=enabled(),state=on?'on':'off';if(card.getAttribute('data-simple-notification')==='1'&&card.getAttribute('data-notification-state')===state){if(typeof forceFeedback==='string')setFeedback(forceFeedback);return}var existing=card.querySelector('#notification-feedback'),feedback=typeof forceFeedback==='string'?forceFeedback:(existing?existing.textContent:'');var c=copy(on);card.className='notification-settings-card notification-settings-simple';card.setAttribute('data-simple-notification','1');card.setAttribute('data-notification-state',state);card.innerHTML='<h1>'+esc(c.title)+'</h1><p>'+esc(c.body)+'</p><button type="button" class="notification-permission-row" data-notification-request aria-label="'+esc(c.label+' '+c.status)+'"><span class="notification-permission-label">'+esc(c.label)+'</span><span class="notification-permission-status '+state+'">'+esc(c.status)+'</span><span class="notification-permission-chevron" aria-hidden="true">›</span></button><div class="settings-action-feedback" id="notification-feedback" aria-live="polite">'+esc(feedback)+'</div><small>'+esc(c.hint)+'</small>'}
