...
function manifestHref(){return location.pathname.indexOf('/about/')>=0?'../manifest.webmanifest':'./manifest.webmanifest'}
function swHref(){return location.pathname.indexOf('/about/')>=0?'../sw.js':'./sw.js'}
function swScope(){return location.pathname.indexOf('/about/')>=0?'../':'./'}
function ensureInstallability(){
  try{if(!document.querySelector('link[rel="manifest"]')){var l=document.createElement('link');l.rel='manifest';l.href=manifestHref();document.head.appendChild(l)}}catch(e){}
  if('serviceWorker'in navigator){navigator.serviceWorker.register(swHref(),{scope:swScope()}).catch(function(){})}
}
