...
function pageTop(){
  ...
  var s=document.scrollingElement;if(s)s.scrollTop=0;
...
function beginEntry(){
  ...
  entryTimer=setTimeout(function(){entryPending=false},1800);
}
...
new MutationObserver(function(){
  if(entryPending&&isRoom()){
    entryPending=false;
    if(entryTimer){clearTimeout(entryTimer);entryTimer=null}
    try{if(document.activeElement&&document.activeElement.blur)document.activeElement.blur()}catch(e){}
    pageTop();
    requestAnimationFrame(pageTop);
    setTimeout(pageTop,80);
    setTimeout(pageTop,240);
  }
}).observe(app,{childList:true,subtree:true});
window.addEventListener('popstate',function(){
  if(isHome())requestAnimationFrame(function(){try{window.scrollTo(0,homeY)}catch(e){}});
});
})();
