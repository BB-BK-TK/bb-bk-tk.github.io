...
function normalizeLegacy(s){
  ...
  s.homeAnswerStatus=null;s.homeAnswerArrived=false;
  if(window.DT&&DT.saveSession)DT.saveSession(s);
  return true;
}
...
function rememberCurrent(){
  ...
  lastSnapshot={state:d,session:s,room:s.room};
}
function persistSnapshot(snapshot){
  if(!snapshot||!window.DT||!DT.saveSession)return false;
  var d=snapshot.state,s=snapshot.session;if(!d||!s)return false;
  var st=answerStatus(d),names=(d.participants||[]).map(function(p){return p.name}).filter(Boolean);
  s.memberNames=names.length?names:s.memberNames;
  s.partnerName=d.partner&&d.partner.name?d.partner.name:s.partnerName;
  ...
  s.homeAnswerRound=d.round_sequence||null;
  DT.saveSession(s);
  return true;
}
...
function paint(card,s){
  ...
  old.innerHTML='<span class="space-status-dot"></span>'+esc(s.homeAnswerStatus);
}
function patchHome(){
  if(patching||!window.DT||!DT.spaces)return;
  var root=document.querySelector('.spaces');
  if(!root){rememberCurrent();return}
  if(lastSnapshot&&persistSnapshot(lastSnapshot))lastSnapshot=null;
  var list=DT.spaces().slice().reverse();list.forEach(normalizeLegacy);var sig=signature(list);
  if(root.getAttribute('data-status-stable-v28')===sig)return;
  patching=true;
  try{
    var cards={};root.querySelectorAll('.spacecard').forEach(function(card){var b=card.querySelector('[data-room]');if(b)cards[b.getAttribute('data-room')]=card});
    list.forEach(function(s){var card=cards[s.room];if(card)paint(card,s)});
    root.setAttribute('data-status-ready','1');
    root.setAttribute('data-space-status-v1',sig);
    root.setAttribute('data-status-stable-v28',sig);
  }finally{patching=false}
}
function start(){
  var app=document.getElementById('app');if(!app)return;
  document.addEventListener('click',function(ev){var n=ev.target&&ev.target.closest&&ev.target.closest('[data-a="home"]');if(n)captureCurrent()},true);
  new MutationObserver(function(){patchHome()}).observe(app,{childList:true,subtree:true});
  rememberCurrent();
  patchHome();
}
