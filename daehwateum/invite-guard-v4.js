...
function render(title,body,button){
  ...
  app.innerHTML='<div class="w"><header class="top"><b class="brand">◉ '+(ko?'도란도란':'Dorandoran')+'</b></header><section class="card intro invite-closed"><div class="orb"><i></i><i></i></div><span class="k">INVITATION</span><h1>'+title+'</h1><p>'+body+'</p><button class="btn full" data-a="cancel">'+button+'</button></section></div>';
}
function run(){
  ...
  fetch('https://kacvynoegfpvgdpqtjdi.supabase.co/rest/v1/rpc/dt_get_invite_status',{
    ...
    .then(function(s){
      ...
      if(s.full){render(ko?'이 도란도란은 다 찼어요.':'This space is full.',ko?'최대 '+s.max_participants+'명까지 함께할 수 있어요. 새로 참여할 수는 없어요.':'Up to '+s.max_participants+' people can join this space. No more spots are available.',ko?'홈으로':'Back home');return}
